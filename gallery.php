<?php 
    include('include/header.php');
?>      <?php 
            include('include/menu.php');
        ?>         
        
        <!--================Banner Area =================Ravichandran-->
        <section class="banner_area">
            <div class="container">
                <div class="banner_content">
                    <h3 title="Gallery"><img class="left_img" src="img/banner/t-left-img.png" alt="">Gallery<img class="right_img" src="img/banner/t-right-img.png" alt=""></h3>
                    <a href="#">Home</a>
                    <a href="#">Gallery</a>
                    <a href="#">Gallery Full Width</a>
                </div>
            </div>
        </section>
        <!--================End Banner Area =================-->
        

        
        <!--================Blog grid Area =================Ravichandran-->
         <section class="blog_grid_area stories_area">
            <div class="container">
                <div class="welcome_title">
                    <h3> Our Galley</h3>
                    <img src="img/w-title-b.png" alt="">
                </div>
                <div class="row">  
                <form>
                      <div class="tab-content pull-right">                                    
                        <div class="col-sm-6 box">
                            <div class="height_item">
                                <select class="selectpicker" placeholder="Selected">
                                    <option>Pondicherry</option>
                                    <option>Tindivanam</option>
                                    <option>Villupuram</option>
                                    <option>Chethpet</option>
                                    <option>Ulunthurpet</option>
                                    <option>Madhagadipet</option>
                                </select>
                            </div>
                        </div>
                    </div>                 
                </form>
                </div><br>
                <div class="row">
                    <div class="blog_grid_inner pagination_area">
                        <div class="col-md-3 col-sm-6">
                            <div class="blog_grid_item">
                                <div class="blog_grid_img">
                                    <img src="img/valli/new_84IMG_20141002_110125.jpg" alt="">
                                </div>
                            </div>
                           <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_2301IMG_0842.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_2407IMG_1676.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_6060IMG_0879.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_6628IMG_0823.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img class="lightbox" src="img/valli/new_7436IMG_0814.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_7481IMG_2002.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_8953IMG_1121.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_9651Image-(14).jpg" alt="">
                                    </div>
                                </div>
                            </div>
                           <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_9399IMG_3835.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_8783IMG_3841.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_8338IMG_3840.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_6628IMG_0823.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img class="lightbox" src="img/valli/new_3201IMG_3827.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_2276IMG_3857.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                            
                            <div class="col-md-3 col-sm-6">
                                <div class="blog_grid_item">
                                    <div class="blog_grid_img">
                                        <img src="img/valli/new_1327IMG_3868.jpg" alt="">
                                    </div>
                                </div>
                            </div>
                    </div>
                </div><br><br>
                <div id="pagination" class="pagination light-theme simple-pagination pull-right">
                <div class="pagination_are">
                    <a class="prev" href="gallery.html">Previous</a>
                    <a class="arrow_left" href="#"><i class="fa fa-angle-left"></i></a>
                    <a class="arrow_right" href="#"><i class="fa fa-angle-right"></i></a>
                    <a class="next" href="gallery_2page.html">Next</a>
                </div>
            </div>
        </section>
        <!--================End Blog grid Area =================-Ravichandran-->
        <?php 
    include('include/footer.php');
?>
        <!--<script src="js/video_player.js"></script>-->
        <script src="js/theme.js"></script>
      <!--   <script>$(function() {
            $('#pagination_are').pagination({
        <script>$(function() {
            $('#pagination')({
                items: 16,
                itemsOnPage : 8,
                cssStyle: 'light-theme'
            });
        });
        </script> -->
    </body>
</html>