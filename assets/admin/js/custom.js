...
$(document).ready(function() {
 ...
 $(function(){
  ...
  $(document).on('blur',".focused",function(e){
  	...
  	$(document).on('change','.filter_act',function(){
  		$('.slidingDiv').hide('slow');
  	});
        // $(".slidingDiv").hide();
        // $('.search_customer').click(function(){
...
//************ Yes button for Delete popup ***********
    ...
    $(document).on('submit','.customer_edit_form,.add_customer_user',function(e) {
        e.preventDefault();
        var error = '';
    ...
    $(document).on('keypress',".dob_admin",function (event) {
  		event.preventDefault();
  	});
	
    $(document).on('change',"#usertype_act",function (e) {
    ...
    $(document).on('change',".period_monthact",function (e) {
    ...
    $(document).on('change',".paymentmode_act",function (e) {
    ...
    $(document).on('change',".paymenttype_act",function (e) {
      // alert("yes");
      var paymenttype = $(this).val();
      // alert(paymenttype);
    });

   $(document).on('keyup',".income-box",function (e) {
    var inc =$(".income-box");
    ...
    $(document).on('keypress',".income-box,.bro_sis,.mob_num,.valli_id,.man_id,.age_reg,.only_num",function (e) {
      // var income =$(this).val();
      // var income =parseInt($(this).val());
      // var s = e.which;
...
//     }
// });
    $(document).on('keypress', '.alp', function() {
         var ew = event.which;
        if(ew == 32)
            return true;
        if(65 <= ew && ew <= 90)
    ...
    // var max_chars = 2;
    $(document).on('keyup', '.bro_sis,.age_reg', function(e) {
    ...
    $(document).on('change','.height_act',function(e){
